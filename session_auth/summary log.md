...
6.8. 인증구현 - 세션저장
- 메모리에 저장된 세션데이터를 저장소에 다가 반영하는 작업을 나중에 함. 저장소가 어떤 이유로 인해 굉장히 느려짐. 한 시간이 걸림. 그 와중 리다이렉션 작업은 순식간에 끝나버리니 아직 세션 스토어에 저장도 안햇는데 리다이렉션이 끝나버리면, 나는 분명히 로그인을 했는데, 로그인이 안되어있다고 화면이 뜨면, 1시간 뒤에 접속해보니 그때 로그인이 되어있다 나올수 도 있다. 
