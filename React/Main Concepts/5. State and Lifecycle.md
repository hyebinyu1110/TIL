...
## The Data Flows Down(데이터의 흐름은 아래로 흐른다)
    - 부모나 자식 컴포넌트 어느 쪽도 특정 컴포넌트가 네트워크 연결 정보를 가지고 있는지 없는지(stateful or stateless) 모릅니다. 
    그래서 부모나 자식 컴포넌트는 컴포넌트가 함수나 클래스로 정의되어있는지 신경쓰면 안됩니다. 
    - 이것이 바로 state가 종종 지역적이다 혹은 캡슐화되어있다 라고 일컬어지는 이유입니다. 컴포넌트가 소유하고 설정하는 컴포넌트가 
    아니라면 다른 컴포넌트에 접속할 수 없습니다.
...
~~~
    ...
    Clock's props로부터 왔는지, 혹은 손으로 쓰여졌는지 모를겁니다.
~~~Java Script
