module.exports = function(app){
  ...
  passport.serializeUser(function(user, done) { 
      // serializeUser는 로그인에 성공했을 때 (딱 한번만 호출됨) 로그인 성공 사실을 세션스토어에 저장하는 기능을 하는 것이 serializeUser이다. 
  ...
  // 사용자가 로그인한 후 각각의 페이지 아무데나 방문할 때마다 우리는 그 사람이 로그인한 사용자인지 아닌지 체크해야 하는데 그걸 체크할 때 , 
  // passport 는 저장된 데이터를 기준으로 해서 우리가 필요한 정보를 조회할때 사용하는 것이 deserializeUser이다. 그래서 그 사용자의 실제 정보를 authData에 넣었음.  
  passport.deserializeUser(function(user, done) {
    console.log('deseralizeUser', user);
    done(null, authData);
  });
